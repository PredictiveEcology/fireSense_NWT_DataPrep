...
write.csv(
  (
    bind_cols(
      ...
      tibble(el = elev[!is.na(elev)]) # Parts of the BCR6 where we have no data, 
...
```{r LCC05_GIS}
#
# Reproject, crop, mask veg data with BCR6
#
## Load BCR6 (raster)
#
BCR6 <- Cache(
...
preCropInputs <- function(bcr6, lcc)
{
  # Speed-up process by pre-cropping: 110% of the extent of BCR6
  e <- as(extent(bcr6) * 1.1, "SpatialPolygons")
  proj4string(e) <- proj4string(bcr6)
  e <- spTransform(e, CRSobj = proj4string(lcc))
  cropInputs(x = lcc, studyArea = e)
}
