...
read_ClimateNA_outputs <- function(files)
{
  ...
  #     function(x)
  #     {
  #       x[x[] == -9999] <- NA # NoData are encoded as -9999
  #       x
  #     }
  #   )
  # )
...
climateNA_outputs_250m -> climateNA_outputs2
  ...
  for (i in 4:9)
  {
    n <- ffvecapply(
      unname(
        days_in_month(
          date(
            paste0(climateNA_outputs[["Year"]][i1:i2], "-0", i, "-01")
          )
        )
      ), 
      X = climateNA_outputs[["Year"]], 
      RETURN = TRUE,
      VMODE = "integer"
    )
    T_mx <- climateNA_outputs[[paste0("Tmax0", i)]] # Max monthly temperature
    r_m <- climateNA_outputs[[paste0("PPT0", i)]]   # Total monthly precip
    E_m <- ffvecapply(n[i1:i2] * (.36 * T_mx[i1:i2] + L_f(i)), X = T_mx, RETURN = TRUE, VMODE = "double")
    RM_EFF <- ffvecapply(.83 * r_m[i1:i2], X = r_m, RETURN = TRUE, VMODE = "double")
    Q_mr <- ffvecapply(3.937 * RM_EFF[i1:i2] / (800 * exp(-MDC_0[i1, i2]/400)), X = T_mx, RETURN = TRUE, VMODE = "double")
    DC_HALF <- ffvecapply(MDC_0[i1:i2] + .25 * E_m, X = T_mx, RETURN = TRUE, VMODE = "double")
    DC_mr <- ffvecapply(DC_HALF[i1:i2] - 400 * log(1 + Q_mr[i1:i2]), X = T_mx, RETURN = TRUE, VMODE = "double")
    MDC_m <- ffvecapply(DC_mr[i1:i2] + .25 * E_m[i1:i2], X = T_mx, RETURN = TRUE, VMODE = "double")
    climateNA_outputs[[paste0("MDC0", i)]] <- MDC_0 <- 
      ffvecapply((MDC_0[i1:i2] + MDC_m[i1:i2]) / 2, X = T_mx, RETURN = TRUE, VMODE = "double")
  }
...
### Produce and save MDC rasters for the 2000-2010 period and the BCR6, 250m resolution
``` {R}
